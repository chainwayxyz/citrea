...
test: $(EF_TESTS_DIR) ## Runs test suite using nextest
	PARALLEL_PROOF_LIMIT=1 cargo nextest run -j30 --locked --workspace --all-features --no-fail-fast $(filter-out $@,$(MAKECMDGOALS))
...
coverage: $(EF_TESTS_DIR) ## Coverage in lcov format
	...
	PARALLEL_PROOF_LIMIT=1 \
	cargo llvm-cov --locked --lcov --output-path lcov.info nextest -j30 --workspace --all-features
