...
describe("RpcTests", function () {
    ...
    it("getCode returns the correct code", async function () {
        ...
        expect(code).to.equal(bitcoinLightClientCode);
    });

    it("getStorage returns the correct storage", async function () {
        // 2nd slot of Bridge is 'operator' and should be the system caller address on its own
    ...
    it("eth_sendRawTransactionSync returns receipt synchronously", async function () {
        const tx = await generateTransaction('10', '0x99');
        const signer = new ethers.Wallet('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80', provider);
        const signedTx = await signer.signTransaction(tx);

        const receipt = await provider.send('eth_sendRawTransactionSync', [signedTx, 5000]);

        expect(receipt.transactionHash).to.exist;
        expect(receipt.from.toLowerCase()).to.equal('0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266');
        expect(receipt.to.toLowerCase()).to.equal('0x3100000000000000000000000000000000000002');
        expect(receipt.status).to.equal("0x1");
        expect(receipt.transactionIndex).to.equal("0x0");
        expect(receipt.l1DiffSize > 0).to.be.true;
        expect(receipt.l1FeeRate > 0).to.be.true;
    });
    ...
    it("eth_sendRawTransactionSync works with default timeout", async function () {
        ...
        // Call without timeout parameter - should use default 2000ms
    ...
    it("eth_sendRawTransactionSync returns timeout error (code 4) with very short timeout", async function () {
        this.timeout(0);
        const tx = await generateTransaction('10', '0x9b');
        const signer = new ethers.Wallet('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80', provider);
        const signedTx = await signer.signTransaction(tx);
        ...
        try {
            ...
            // EIP-7966 error code 4: timeout
            expect(error.error.code).to.equal(4);
    ...
    it("eth_sendRawTransactionSync rejects invalid transaction", async function () {
        const invalidTx = '0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef';
        try {
            await provider.send('eth_sendRawTransactionSync', [invalidTx]);
            expect.fail('Expected error for invalid transaction');
        } catch (error) {
            expect(error.error.code).to.equal(-32602);
            expect(error.error.message).to.equal('failed to decode signed transaction');
        }
    });
