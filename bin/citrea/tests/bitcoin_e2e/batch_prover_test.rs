    ...
    async fn run_test(&mut self, f: &mut TestFramework) -> Result<()> {
        ...
        let commitments: Vec<SequencerCommitment> = full_node
            ...
            .map(|response| SequencerCommitment {
                ...
                l2_start_block_number: response.l2_start_block_number.try_into().expect("U64 to u64 must succeed"),
                l2_end_block_number: response.l2_end_block_number.try_into().expect("U64 to u64 must succeed"),
            })
            .collect();
...
impl TestCase for ForkElfSwitchingTest {
    ...
    async fn run_test(&mut self, f: &mut TestFramework) -> Result<()> {
        ...
        assert_eq!(
            fork_from_block_number(proofs[0].proof_output.last_l2_height.try_into().expect("U64 to u64 must succeed")).spec_id,
            SpecId::Genesis
        );
        assert_eq!(
            fork_from_block_number(proofs[1].proof_output.last_l2_height.try_into().expect("U64 to u64 must succeed")).spec_id,
            SpecId::Kumquat
        );
