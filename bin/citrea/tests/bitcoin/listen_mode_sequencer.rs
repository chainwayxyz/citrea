...
struct ReadOnlySequencerRpcToggleTest;

/*
Same as `ReadOnlySequencerTest` through step 11, but instead of reviving the read-only sequencer by
restarting it with the main sequencer's config, it is promoted to producer **in-process** via the
`citrea_convertToProducer` RPC (no restart, no address takeover).
...
10. Verify that the read-only sequencer cannot fetch it because it is not finalized yet
11. Shut Down main sequencer and full node.
12. Call the RPC of the read-only sequencer to toggle from listen mode to producer
13. Verify that after toggle the read-only sequencer does have the non-finalized commitment
14. Publish more l2 blocks from the toggled sequencer with transactions
15. Restart full node pointed at the toggled sequencer's url
16. See that full node can sync properly
17. Check the read-only sequencer historical state works as intended
18. Send commitment from toggled sequencer and get it finalized
19. Verify that full node can fetch the finalized commitment and verify it
20. Roll back the toggled sequencer and full node to a previous state
21. Publish more l2 blocks and still see full node can sync with toggled sequencer
*/
#[async_trait]
impl TestCase for ReadOnlySequencerRpcToggleTest {
    fn test_config() -> TestCaseConfig {
        TestCaseConfig {
            n_nodes: HashMap::from([(NodeKind::Sequencer, 2)]),
            with_sequencer: true,
    ...
    async fn run_test(&mut self, f: &mut TestFramework) -> Result<()> {
        ...
        // Promote the read-only sequencer to producer. The convert handler probes the (now-dead)
        // main sequencer, sees it is unreachable, and starts producing from the synced state. The
        // read-only sequencer keeps its own rpc address, so its client needs no repointing.
        readonly_sequencer
            .client
            .http_client()
            .convert_to_producer()
        ...
        .await?;
        // Now the readonly sequencer is the main sequencer
        // Publish some blocks from the toggled sequencer
        for _ in 0..max_l2_blocks_per_commitment / 2 {
            let _ = readonly_sequencer_test_client
                .send_eth(some_address, None, None, None, 1e18 as u128)
                .await
        ...
        // Start the full node pointed at the toggled sequencer's url (it kept its own address, so
        // unlike the restart-based test it did not take over the main sequencer's address).
        let mut full_node_config = full_node.config.clone();
        if let Some(runner) = full_node_config.rollup.runner.as_mut() {
            runner.sequencer_client_url = format!(
                "http://{}:{}",
                readonly_sequencer.config.rollup.rpc.bind_host,
                readonly_sequencer.config.rollup.rpc.bind_port,
            );
        }
        ...
        // Wait for full node to sync with the toggled sequencer
        full_node
            .wait_for_l2_height(head_l2_height.to::<u64>(), None)
            .await?;
        ...
        // Check the historical balance of the address before it was toggled
        let historical_balance = readonly_sequencer_test_client
            .eth_get_balance(some_address, Some(BlockId::earliest()))
            .await
            .unwrap();
        ...
        // Also see that the toggled sequencer can see the non-finalized commitment
        let toggled_commitment = readonly_sequencer
            .client
            .http_client()
            .get_sequencer_commitment_by_index(U32::from(2))
            .await?;
        assert!(toggled_commitment.is_some());
        ...
        // Now publish more l2 blocks and see that toggled sequencer can send commitments
        for _ in 0..max_l2_blocks_per_commitment {
            readonly_sequencer
                .client
                .send_publish_batch_request()
        ...
        let citrea_cli = f.citrea_cli.as_ref().unwrap();
        // Rollback the toggled sequencer, full node and da to a previous state
        citrea_cli
            .run(
                "rollback",
                &[
        ...
        // Restart the toggled sequencer as a producer. The in-process toggle did not change the
        // harness config, which still has `listen_mode_config` set, so clear it to come back up
        ...
        // Publish more l2 blocks and see that full node can still sync with toggled sequencer
        for _ in 0..max_l2_blocks_per_commitment {
            readonly_sequencer
                .client
                .send_publish_batch_request()
...
#[tokio::test]
async fn read_only_sequencer_rpc_toggle_test() -> Result<()> {
    TestCaseRunner::new(ReadOnlySequencerRpcToggleTest)
        .set_citrea_path(get_citrea_path())
        .run()
        .await
}
