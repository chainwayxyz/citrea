...
Mac:
```sh
./citrea-v2.7.0-osx-arm64 --network mainnet --da-layer bitcoin --rollup-config-path ./rollup_config.toml --genesis-paths ./genesis
```
...
RUST_LOG=info \
JSON_LOGS=1 \
./citrea-v2.7.0-osx-arm64 --network mainnet --da-layer bitcoin --genesis-paths ./genesis
```

Linux:
```sh
./citrea-v2.7.0-linux-amd64 --network mainnet --da-layer bitcoin --rollup-config-path ./rollup_config.toml --genesis-paths ./genesis
```
...
RUST_LOG=info \
JSON_LOGS=1 \
./citrea-v2.7.0-linux-amd64 --network mainnet --da-layer bitcoin --genesis-paths ./genesis
```
