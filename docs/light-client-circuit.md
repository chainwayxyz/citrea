...
The light client proof's main use case is to provide the L2 state root to the Citrea's Bitcoin Bridge, [Clementine](https://citrea.xyz/clementine_whitepaper.pdf). The Light Client Proof is verified inside Clementine's Bridge Proof in case of a challenge to one of the bridge operators
