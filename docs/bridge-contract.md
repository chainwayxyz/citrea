...
3.  **Relaying to Citrea**: The bridge operator monitors the Bitcoin network. Once the move transaction is confirmed, the operator calls the `deposit()` function on `Bridge.sol`.
