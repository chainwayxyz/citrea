...
4. **Completion of the Withdrawal**: After users calling the `withdraw()` function bridge operators compete to fulfill this withdraw request. Users can decrease the amount that they want in the Bitcoin side via changing signatures in an dutch auction fashion if there are no operators accepting at that price.
