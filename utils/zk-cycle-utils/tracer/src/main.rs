...
fn cycle_count(insn: &str) -> Result<usize, &'static str> {
    ...
    match insn {
        "LB" | "LH" | "LW" | "LBU" | "LHU" | "ADDI" | "SLLI" | "SLTI" | "SLTIU" | "AUIPC"
        | "SB" | "SH" | "SW" | "ADD" | "SUB" | "SLL" | "SLT" | "SLTU" | "XOR" | "SRL" | "SRA"
        | "OR" | "AND" | "MUL" | "MULH" | "MULSU" | "MULU" | "LUI" | "BEQ" | "BNE" | "BLT"
        | "BGE" | "BLTU" | "BGEU" | "JALR" | "JAL" | "ECALL" | "EBREAK" => Ok(1),
...
fn focused_stack_counts(
    ...
    if let Some(index) = function_stack.iter().position(|s| s == function_name) {
        ...
        let count = filtered_stack_counts
            ...
            .or_insert(0);
        *count += cycle_count(instruction).unwrap();
    }
}
...
fn increment_stack_counts(
    ...
    for f in function_stack {
        *instruction_counts.entry(f.clone()).or_insert(0) += cycle_count(instruction).unwrap();
    }
...
fn main() -> std::io::Result<()> {
    ...
    for (c, line) in file_content.lines().enumerate() {
        ...
        if let Ok(index) = function_ranges.binary_search_by(|&(start, end, _)| {
            ...
            *counts_without_callgraph.entry(fname.clone()).or_insert(0) +=
                cycle_count(instruction).unwrap();
        } else {
            *counts_without_callgraph
                .entry("anonymous".to_string())
                .or_insert(0) += cycle_count(instruction).unwrap();
        }
