...
impl Api for Server {
    ...
    fn get_block(
        ...
        if verbosity == 0 {
            ...
            Err(jsonrpc_core::Error::new(
                jsonrpc_core::types::error::ErrorCode::InvalidParams,
            ))
        }
    }
    ...
    fn get_transaction(
        ...
        match self.state.lock().unwrap().transactions.get(&txid) {
            Some(tx) => Ok(serde_json::to_value(GetTransactionResult {
                info: WalletTxInfo {
                    txid,
                    confirmations: 0,
                    time: 0,
    ...
    fn get_raw_transaction(
        ...
        if verbose.unwrap_or(false) {
            match self.state().transactions.get(&txid) {
                Some(_) => Ok(serde_json::to_value(GetRawTransactionResult {
