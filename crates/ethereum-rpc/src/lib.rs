    ...
    Da: DaService,
{
    ...
    fn eth_get_proof(
        ...
        let block_id_internal = evm.block_number_from_state(block_id, &mut working_set, &self.ethereum.ledger_db)?;

        evm.set_state_to_end_of_evm_block_by_block_id(block_id, &mut working_set, &self.ethereum.ledger_db)?;
