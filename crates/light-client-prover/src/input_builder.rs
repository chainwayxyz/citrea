...
pub(crate) fn build_circuit_input_from_l1_block<Da, DB, Vm>(
    network: Network,
    ...
    Network: InitialValueProvider<Da::Spec>,
{
    ...
    let RunL1BlockResult {
        ...
        previous_lcp_output,
        network.get_l2_genesis_root(),
        network.initial_batch_proof_method_ids().to_vec(),
        &network.batch_prover_da_public_key(),
        &network.sequencer_da_public_key(),
        &network.method_id_upgrade_authority_da_public_keys(),
    );
