...
#[test]
fn db_compact() {
    let path = Path::new("/Users/erce/Desktop/Chainway/secret-sovereign-sdk/resources/dbs");
    // let pathbuf = path.clone().to_path_buf();
    let cfg = &RocksdbConfig::new(path, None);
    let path = cfg.path.join(LEDGER_DB_PATH_SUFFIX);
    let raw_options = cfg.as_raw_options(false);
    let mut updated_tables = LEDGER_TABLES.to_vec();
    let cfds = updated_tables.iter().copied().into_iter().map(|cf_name| {
        println!("cf name:{:?}", cf_name);
        let mut cf_opts = rocksdb::Options::default();
        cf_opts.set_compression_type(rocksdb::DBCompressionType::Lz4);
        cf_opts.set_block_based_table_factory(&raw_options.block_options);
        rocksdb::ColumnFamilyDescriptor::new(cf_name, cf_opts)
    });
    let mut db = rocksdb::DB::open_cf_descriptors(&raw_options.db_options, path, cfds).unwrap();
...
#[test]
fn list_cfs() {
    let path = Path::new("/Users/erce/Desktop/Chainway/secret-sovereign-sdk/resources/dbs/ledger");
    let cfs = rocksdb::DB::list_cf(&rocksdb::Options::default(), path)
        .expect("Failed to list column families");
    println!("Column families: {:?}", cfs);
}

#[test]
fn test_drop_test_cfs() {
    let path = Path::new("/Users/erce/Desktop/Chainway/secret-sovereign-sdk/resources/dbs");
    // let pathbuf = path.clone().to_path_buf();
    let cfg = &RocksdbConfig::new(path, None);
    let path = cfg.path.join(LEDGER_DB_PATH_SUFFIX);
    ...
    assert!(db.cf_handle("TestTableOld").is_some());
    assert!(db.cf_handle("TestTableNew").is_some());

    db.drop_cf("TestTableOld").unwrap();
    db.drop_cf("TestTableNew").unwrap();

    assert!(db.cf_handle("TestTableOld").is_none());
    assert!(db.cf_handle("TestTableNew").is_none());
}
...
fn test_drop_cf() {
    ...
    let path = cfg.path.join(LEDGER_DB_PATH_SUFFIX);
    let raw_options = cfg.as_raw_options(false);
    let old_table = "VerifiedProofsBySlotNumber";
    let new_table = "VerifiedBatchProofsBySlotNumber";
    let mut updated_tables = LEDGER_TABLES.to_vec();
    updated_tables.push(&old_table);
    let cfds = updated_tables.iter().copied().into_iter().map(|cf_name| {
        println!("cf name:{:?}", cf_name);
        let mut cf_opts = rocksdb::Options::default();
        cf_opts.set_compression_type(rocksdb::DBCompressionType::Lz4);
    ...
    assert!(db.cf_handle("VerifiedProofsBySlotNumber").is_some());

    db.drop_cf("VerifiedProofsBySlotNumber").unwrap();

    assert!(db.cf_handle("VerifiedProofsBySlotNumber").is_none());
}
...
fn test_m() {
    ...
    let db = rocksdb::DB::open_cf_descriptors(&raw_options.db_options, path, cfds).unwrap();

    let cf1 = db
        .cf_handle("VerifiedProofsBySlotNumber")
        .expect("VerifiedProofsBySlotNumber not found");

    let cf_list = db.get_column_family_metadata_cf(cf1);

    println!("Found column family: {:?}", cf_list);

    let cf2 = db
        .cf_handle("VerifiedBatchProofsBySlotNumber")
        .expect("VerifiedBatchProofsBySlotNumber");

    let cf3 = db
        .cf_handle("SoftConfirmationByNumber")
        .expect("SoftConfirmationByNumber");

    // let pinned_cf = db.get_pinned_cf(cf1, key)

    // Iterate over all key-value pairs in cf1 and insert them into cf2
    let iter = db.iterator_cf(cf1, rocksdb::IteratorMode::Start);

    let iter3 = db.iterator_cf(cf3, rocksdb::IteratorMode::Start);

    let mut ct = 0;
    for res in iter {
        let (key, value) = res.unwrap();
        db.put_cf(cf2, &key, &value).expect("Write failed");
        ct += 1;
    }
    ...
    //     db: Arc::new(db),
    //     next_item_numbers: Arc::new(Mutex::new(ItemNumbers::default())),
    // };

    // // let ledger_db = LedgerDB::with_config(&cfg).unwrap();
    // let slot = ledger_db
    //     .get_verified_proof_data_by_l1_height(1)
    //     .unwrap()
    //     .unwrap();
    // println!("\n\n\nslot: {:?}", slot);
    ///////////

    // let original_path = db.path();
