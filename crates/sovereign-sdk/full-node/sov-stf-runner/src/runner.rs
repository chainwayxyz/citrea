            ...
            if last_l1_height != soft_batch.da_slot_height || cur_l1_block.is_none() {
                ...
                for proof in zk_proofs {
                                ...
                                >(&proof, &self.code_commitment)
                            {
                                    ...
                                    || proof_data.sequencer_public_key != self.sequencer_pub_key
                                {
                                    tracing::warn!(
                                        ...
                                        Sequencer public key or sequencer da public key mismatch,
                                        Skipping to next proof.."
                                    );
                                    continue;
                                }
                                proof_data
                                ...
                                tracing::warn!(
                                    "Proof verification failed, Skipping to next proof.."
                                );
                                continue;
                            }
                        }
                    ...
                    let l1_height = match self.ledger_db.get_l1_height_of_l1_hash(l1_hash)? {
                        ...
                        None => {
                            tracing::warn!("L1 height not found for l1 hash: {:?}", l1_hash);
                            continue;
                        }
                    };
                    ...
                    let l1_height_start =
                        match self.ledger_db.get_l1_height_of_l1_hash(first_slot_hash)? {
                            Some(l1_height) => l1_height,
                            None => {
                                tracing::warn!("L1 height not found for l1 hash: {:?}", l1_hash);
                                continue;
                        ...
                        .get_l2_range_by_l1_height(SlotNumber(l1_height_start))?
                    {
                        Some((start, end)) => {
                            ...
                            {
                                tracing::warn!(
                                    "Pre state root mismatch - expected 0x{} but got 0x{}",
                        ...
                        None => {
                            tracing::warn!(
                                "L1 L2 connection does not exist. L1 height = {}",
                                l1_height_start
                            );
                            continue;
                        }
