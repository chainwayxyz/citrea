...
#[proc_macro_derive(ModuleInfo, attributes(state, module, address, gas, memory))]
...
///
/// impl<C: Context> MyModule<C> {
///     fn my_method(&self, working_set: &mut WorkingSet<<C as Spec>::Storage>, param: u32) -> RpcResult<u32> {
///         Ok(1)
///     }  
/// }
///
...
/// by simply implementing the two required methods - `get_backing_impl(&self) -> MyModule` and `get_working_set(&self) -> ::sov_modules_api::WorkingSet<<C as Spec>::Storage>`
///
/// ```rust,ignore
/// pub trait MyModuleRpcImpl<C: sov_modules_api::Context> {
///     fn get_backing_impl(&self) -> &TestStruct<C>;
///     fn get_working_set(&self) -> ::sov_modules_api::WorkingSet<<C as Spec>::Storage>;
