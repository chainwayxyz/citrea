...
fn test_fork_pos_from_block_number() {
    static T_FORKS: &[Fork] = &[
        ...
        Fork::new(SpecId::Tangelo, 100),
        Fork::new(SpecId::Tangelo30M, 200),
    ...
    assert_eq!(fork_pos_from_block_number(T_FORKS, 350), 2);
    assert_eq!(fork_pos_from_block_number(T_FORKS, 505), 3);
}
...
fn test_fork_manager() {
    ...
    assert_eq!(fork_manager.active_fork().spec_id, SpecId::Tangelo);
    fork_manager.register_block(350).unwrap();
    assert_eq!(fork_manager.active_fork().spec_id, SpecId::Tangelo30M);
...
fn test_fork_manager_callbacks() {
    ...
    impl ForkMigration for Handler {
        fn fork_activated(&self, fork: &Fork) -> anyhow::Result<()> {
            if fork.spec_id == SpecId::Tangelo {
