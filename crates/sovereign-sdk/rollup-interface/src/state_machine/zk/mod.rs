...
pub struct StateTransitionData<StateRoot, Witness, Da: DaSpec> {
    ...
    /// The sequencer commitments (and zk proofs, those will be ignored) that are being processed
    pub sequencer_commitments: Vec<DaData>,
    ...
    pub da_inclusion_proof: Da::InclusionMultiProof,
    /// The completeness proof for all DA data.
    pub da_completeness_proof: Da::CompletenessProof,

    /// The soft confirmations that are inside the sequencer commitments.
    pub soft_confirmations: Vec<Vec<SignedSoftConfirmationBatch>>,
    /// Corresponding witness for the soft confirmations.
    pub state_transition_witnesses: Vec<Vec<Witness>>,
    /// DA block headers the soft confirmations was constructed on.
    pub da_block_headers_of_soft_confirmations: Vec<Vec<Da::BlockHeader>>,
}
