...
pub trait ZkvmHost: Zkvm + Clone {
    ...
    /// Waits for a receipt to be generated by the zkVM program.
    fn wait_for_receipt(&self, session: &String) -> Result<Vec<u8>, anyhow::Error>;

    /// Creates a new snark session in the zkVM program.
    fn create_new_snark_session(&self, session: &String) -> Result<String, anyhow::Error>;

    /// Waits for the zkVM program to convert a Stark proof to a SNARK proof.
