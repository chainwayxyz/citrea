...
impl TransactionV1 {
    fn verify(&self) -> anyhow::Result<()> {
        ...
        let mut serialized_tx = Vec::with_capacity(self.runtime_msg.len() + 2 * size_of::<u64>());
...
impl TransactionV2 {
    #[cfg(feature = "native")]
    fn new(priv_key: &K256PrivateKey, runtime_msg: Vec<u8>, nonce: u64) -> Self {
        let mut message = Vec::with_capacity(runtime_msg.len() + size_of::<u64>());
    ...
    fn verify(&self) -> anyhow::Result<()> {
        let mut serialized_tx = Vec::with_capacity(self.runtime_msg.len() + size_of::<u64>());
...
impl Transaction {
    ...
    #[cfg(feature = "native")]
    pub fn new_signed_tx(priv_key: &K256PrivateKey, runtime_msg: Vec<u8>, nonce: u64) -> Self {
        Self::V2(TransactionV2::new(priv_key, runtime_msg, nonce))
    }
