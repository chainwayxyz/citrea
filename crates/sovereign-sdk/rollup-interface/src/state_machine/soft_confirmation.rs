...
/// Contains the signature and public key of the sequencer
#[derive(PartialEq, Eq, BorshDeserialize, BorshSerialize, Serialize, Deserialize)]
pub struct SignedSoftConfirmationV2<'txs, Tx: Clone> {
    l2_height: u64,
    hash: [u8; 32],
    prev_hash: [u8; 32],
    da_slot_height: u64,
...
impl<'txs, Tx: Clone> SignedSoftConfirmationV2<'txs, Tx> {
    /// Creates a signed soft confirmation batch
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        l2_height: u64,
...
impl<'txs, Tx: Clone, TxOld: Clone + From<Tx>> From<SignedSoftConfirmation<'txs, Tx>>
    for SignedSoftConfirmationV2<'txs, TxOld>
{
    fn from(input: SignedSoftConfirmation<'txs, Tx>) -> Self {
        SignedSoftConfirmationV2 {
            l2_height: input.l2_height,
            hash: input.hash,
            prev_hash: input.prev_hash,
            da_slot_height: input.da_slot_height,
...
impl<'txs, TxOld: Clone, Tx: Clone + From<TxOld>> From<SignedSoftConfirmationV2<'txs, TxOld>>
    for SignedSoftConfirmation<'txs, Tx>
{
    fn from(input: SignedSoftConfirmationV2<'txs, TxOld>) -> Self {
