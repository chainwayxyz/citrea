    ...
    DB: BatchProverLedgerOps,
{
    ...
    commitment_tx: mpsc::Receiver<Vec<SequencerCommitment>>,
}

impl<Vm, Da, DB> Prover<Vm, Da, DB>
where
    Da: DaService,
    Vm: ZkvmHost + Zkvm,
    DB: BatchProverLedgerOps + Clone + 'static,
{
    ...
    pub async fn run_new(mut self, cancellation_token: CancellationToken) {
        loop {
            select! {
                ...
                Some(commitments) = self.commitment_tx.recv() => {
                    if let Err(e) = self.process_commitments(commitments).await {
    ...
    async fn process_commitments(&mut self, commitments: Vec<SequencerCommitment>) -> anyhow::Result<()> {
        Ok(())
    }
