...
pub async fn build_services<Da, DB, RT, Vm, Witness>(
    ...
    Witness: Default + BorshSerialize + BorshDeserialize + Serialize + DeserializeOwned,
{
    ...
    let rpc_context = rpc::create_rpc_context::<Da, Vm, DB, RT>(
        da_service.clone(),
    ...
    let rpc_module = rpc::register_rpc_methods::<Da, Vm, DB, RT>(rpc_context, rpc_module)?;
