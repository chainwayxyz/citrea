...
impl<C: sov_modules_api::Context> Evm<C> {
    ...
    ) -> RpcResult<Option<WithOtherFields<RpcBlock<AnyNetwork>>>> {
    ...
    pub fn get_block_by_number(
        ...
        // TODO: Should I leave just default uncles instead of this?
        let uncles = primitive_block
            .body
            .ommers
            .into_iter()
            .map(|h| h.hash_slow())
    ...
    pub fn get_block_receipts(
        ...
        working_set: &mut WorkingSet<C>,
    ) -> RpcResult<Option<Vec<RpcReceipt<AnyNetwork>>>> {
    ...
    pub fn get_transaction_by_block_hash_and_index(
        ...
        // TODO: Deduplicate with `get_transaction_by_block_number_and_index`
        ...
        match check_tx_range(&block.transactions, index.into()) {
            Some(_) => (),
            None => return Ok(None),
        }
        let index = index.0 as u64;
        let tx_number = block.transactions.start + index;
    ...
    pub fn get_transaction_by_block_number_and_index(
        ...
        let block_number = match self.block_number_for_id(&block_number.into(), working_set) {
            Ok(block_number) => block_number,
            Err(EthApiError::HeaderNotFound(block_id)) => return Ok(None),
    ...
    pub fn get_transaction_receipt(
        ...
        hash: B256,
        working_set: &mut WorkingSet<C>,
    ) -> RpcResult<Option<RpcReceipt<AnyNetwork>>> {
        let mut accessory_state = working_set.accessory_state();
    ...
    pub fn get_call(
        &self,
        request: alloy_rpc_types::eth::TransactionRequest,
        block_id: Option<BlockId>,
            ...
            let block_env = match block_number {
                ...
                _ => {
                    let block = self
                        ...
                        .ok_or(EthApiError::HeaderNotFound(block_id.expect("Should get latest block if block id is None")))?;
                    BlockEnv::from(&block)
                }
            };
            ...
            let (l1_fee_rate, block_env) = match block_number {
                ...
                _ => {
                    let block = self
                        ...
                        .ok_or(EthApiError::HeaderNotFound(block_number.map(Into::into).expect("Should get latest block if block id is None")))?;
    ...
    fn estimate_tx_expenses(
        ...
        let (l1_fee_rate, block_env, cfg_env) = {
            let (l1_fee_rate, block_env) = match block_id {
                ...
                Some(BlockId::Number(block_number)) => {
                    match block_number {
                        ...
                        _ => {
                            let block = self
                                .get_sealed_block_by_number(block_number.clone().into(), working_set)?
                                .ok_or(EthApiError::HeaderNotFound(block_id.expect("Block id is not None")))?;
                            (block.l1_fee_rate, BlockEnv::from(&block))
                        }
                    }
                ...
                _ => {
                    let block = self
                        .get_sealed_block_by_number(None, working_set)?
                        .ok_or(EthApiError::HeaderNotFound(block_id.expect("If block id is None should get latest block and not fail")))?;
                    (block.l1_fee_rate, BlockEnv::from(&block))}
            };
    ...
    pub fn eth_estimate_diff_size(
        ...
        let estimated = self.estimate_tx_expenses(request, block_number.map(Into::into), working_set)?;
    ...
    pub fn block_number_for_id(
        ...
        match block_id {
            ...
            match num {
                ...
                BlockNumberOrTag::Pending => Err(EthApiError::HeaderNotFound(block_id.clone())),
                BlockNumberOrTag::Number(block_number) => {
                    if *block_number < self.blocks.len(&mut working_set.accessory_state()) as u64 {
                        Ok(*block_number)
                    } else {
                        Err(EthApiError::HeaderNotFound(block_id.clone()))
                    }
                }
                ...
                match block_num {
                    BlockNumberOrTag::Number(num) => {
                        ...
                        if num > curr_block_number {
                            return Err(EthApiError::HeaderNotFound(block_id.expect("If block id is None should get latest block without fail")));
                        }
...
fn check_tx_range(transactions_range: &Range<u64>, index: usize) -> Option<()> {
    let range_len = transactions_range.end - transactions_range.start;
    if index as u64 >= range_len {
        None
    } else {
        Some(())
    }
