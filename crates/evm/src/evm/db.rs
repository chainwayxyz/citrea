...
pub trait IsAccountNewlyCreatedProvider {
    /// Check if an account is newly created
    /// By querying `Evm::account_exists`
    fn is_account_new(&mut self, address: &Address) -> bool;
}

impl<C: sov_modules_api::Context> IsAccountNewlyCreatedProvider for EvmDb<'_, C> {
    fn is_account_new(&mut self, address: &Address) -> bool {
...
impl<C: sov_modules_api::Context> IsAccountNewlyCreatedProvider for &mut EvmDb<'_, C> {
    fn is_account_new(&mut self, address: &Address) -> bool {
        // As the diff size is calculated in `Handler::output` before `DataBase::commit`,
        // We wouldn't have them in the account indices map
        // So this can tell us if the account is newly created
...
pub mod immutable {
    ...
    use super::{DBError, EvmDb, IsAccountNewlyCreatedProvider};
    ...
    impl<C: sov_modules_api::Context> IsAccountNewlyCreatedProvider for &mut EvmDbRef<'_, '_, C> {
        fn is_account_new(&mut self, address: &Address) -> bool {
