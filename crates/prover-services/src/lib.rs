...
/// A proof along with proving info and the time it took to generate it.
/// This is used so we can save proving info and label the duration metric
/// by node type (batch prover vs light client prover).
#[derive(Debug)]
