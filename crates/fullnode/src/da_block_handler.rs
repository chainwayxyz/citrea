        ...
        + Debug,
{
    ...
    async fn process_zk_proof(
        ...
        let (
            ...
            batch_proof_sequencer_public_key,
            batch_proof_last_l2_height,
            batch_proof_initial_state_root,
            batch_proof_final_state_root,
            batch_proof_state_diff,
            batch_proof_da_slot_hash,
            batch_proof_sequencer_commitments_range,
            batch_proof_preproven_commitments,
        ...
        if batch_proof_sequencer_da_public_key != self.sequencer_da_pub_key
            || batch_proof_sequencer_public_key != self.sequencer_pub_key
        {
            return Err(anyhow!(
        ...
        let last_active_spec_id = fork_from_block_number(FORKS, batch_proof_last_l2_height).spec_id;
        ...
        let stored_batch_proof_output = StoredBatchProofOutput {
            initial_state_root: batch_proof_initial_state_root.as_ref().to_vec(),
            final_state_root: batch_proof_final_state_root.as_ref().to_vec(),
            state_diff: batch_proof_state_diff,
            da_slot_hash: batch_proof_da_slot_hash.clone().into(),
            sequencer_commitments_range: batch_proof_sequencer_commitments_range,
            sequencer_public_key: batch_proof_sequencer_public_key,
            sequencer_da_public_key: batch_proof_sequencer_da_public_key,
            preproven_commitments: batch_proof_preproven_commitments.clone(),
        };

        let l1_hash = batch_proof_da_slot_hash.into();
        ...
        let excluded_commitment_indices = batch_proof_preproven_commitments.clone();
        ...
        let l2_height = filtered_commitments[batch_proof_sequencer_commitments_range.0 as usize]
        ...
        if prior_soft_confirmation_post_state_root.as_ref()
            != batch_proof_initial_state_root.as_ref()
        {
            return Err(anyhow!(
                    ...
                    hex::encode(&batch_proof_initial_state_root)
                ).into());
        }

        for commitment in filtered_commitments
            .iter()
            .skip(batch_proof_sequencer_commitments_range.0 as usize)
            .take(
                (batch_proof_sequencer_commitments_range.1
                    - batch_proof_sequencer_commitments_range.0
                    + 1) as usize,
            )
        {
